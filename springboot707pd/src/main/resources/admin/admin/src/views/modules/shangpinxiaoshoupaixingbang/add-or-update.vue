<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="月销售量" prop="yuexiaoshouliang">
          <el-input v-model="ruleForm.yuexiaoshouliang" 
              placeholder="月销售量" clearable  :readonly="ro.yuexiaoshouliang"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="月销售量" prop="yuexiaoshouliang">
              <el-input v-model="ruleForm.yuexiaoshouliang" 
                placeholder="月销售量" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.shangpintupian = true;
          ...
          if(o=='yuexiaoshouliang'){
            this.ruleForm.yuexiaoshouliang = obj[o];
	    this.ro.yuexiaoshouliang = true;
            continue;
          }
        }
      }
    ...
    info(id) {
      this.$http({
        url: `shangpinxiaoshoupaixingbang/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `shangpinxiaoshoupaixingbang/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.shangpinxiaoshoupaixingbangCrossAddOrUpdateFlag = false;
                  this.parent.search();
